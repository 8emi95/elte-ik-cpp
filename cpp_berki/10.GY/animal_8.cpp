...
class Frog : public virtual Animal {
public:
...
public:
	Husky(std::string name) : Dog(name), Animal(name, 4) {
...
public:
	HuskyFrog(std::string name) : Husky(name), Frog("Fred"), Animal(name, 2) {
		std::cout<<Animal::_name<<" HuskyFrog()"<<std::endl; }
	~HuskyFrog() { std::cout<<Animal::_name<<" ~HuskyFrog()"<<std::endl; }
	void speak() const {}
};
...
int main() 
{
	...
	roger.shakePaw();
	std::cout<<roger.getLegs()<<std::endl;
}
